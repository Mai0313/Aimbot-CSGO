...
def build_engines_initial(weights: str) -> None:
    for file in weights:
        ...
        if engine_filename not in engines:
            ...
            print(
                f"[red]{engine_filename} is missing.[/red] [magenta]Building engine from YOLO weights. This may take a while..."
            )
            ...
            build_engine(
                os.path.join(model_path, pt_filename),
                input_shape,
                os.path.join(model_path, pkl_filename),
            )
...
def run_aimaide(*args):
    ...
    if infer.lower() == "yolov8":
        ...
        Aim = AimAideTrt(
            ...
            model_path=os.path.join(model_path, model),
            model_config_path=model_config_path,
            no_engine_check=no_engine_check,
        )
...
with gr.Blocks() as app:
    ...
    infer = gr.Radio(
        label="Inference",
        choices=["TensorRT", "YOLOv8"],
        info="TensorRT is recommended. If TensorRT is not working on your machine you can always fall back to YOLOv8.",
    )
    model = gr.Dropdown(
        label="Model",
        choices=[],
        interactive=True,
        info="List of all available models in your /models folder",
    )
    grabber = gr.Radio(
        label="Image grabber",
        choices=["win32", "d3d_cpu", "d3d_gpu"],
        value="win32",
        info="d3d_gpu is not working right now!",
    )

    infer_map = {"YOLOv8": weights, "TensorRT": engines}

    def filter_models(infer):
        if len(engines) == 0 and infer == "TensorRT" and not trt_err_flag:
            build_engines_initial(weights)

        return gr.Dropdown.update(choices=infer_map[infer]), gr.update(visible=True)

    def filter_value(model):
        if model in ():
            return gr.update(maximum=0.99)
        else:
            return gr.update(maximum=0.99)
    ...
    with gr.Column(visible=True) as options:
        side = gr.Radio(
            label="Player side",
            choices=["CT", "T", "Deathmatch"],
            value="CT",
            info="Deathmatch locks onto CT and T.",
        )
        minconf = gr.Slider(
            0.2,
            0.99,
            ...
            interactive=True,
            info="Lower value = more detections, more false positives | Higher value = less detections, less false positives",
        )
        sense = gr.Slider(
            1,
            16,
            ...
            interactive=True,
            info="Only increase if AimAide behaves erraticaly and fine-tuning in CS:GO is not fixing it.",
        )
        flick = gr.Slider(
            4,
            16,
            ...
            label="Flickieness",
            interactive=True,
            info="Lower Value = less flicky | Higher value = more flicky",
        )
        with gr.Row():
            ...
            benchmark = gr.Checkbox(
                ...
                info="Runs in view only mode for 300 iterations and returns the average FPS.",
            )
            no_engine_check = gr.Checkbox(
                label="No engine check",
                info="Disables engine checking when running with TensorRT. (check if you encounter problems with TensorRT while building engines)",
            )
        custom_config = gr.Textbox(
            label="Path to custom model config file",
            info="If you want to use your own model, you can write a custom model config file and link it here. (see /models/config.json)",
        )
