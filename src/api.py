...
def Conv(
    ...
    dot_product = network.add_elementwise(
        conv.get_output(0), sigmoid.get_output(0), trt.ElementWiseOperation.PROD
    )
...
def Bottleneck(
    ...
    conv2 = Conv(network, weights, conv1.get_output(0), c2, 3, 1, group, layer_name + ".cv2")
...
def C2f(
    ...
    for i in range(n):
        b = Bottleneck(
            network, weights, y2, c_, c_, shortcut, group, 1.0, layer_name + ".m." + str(i)
        )
        y2 = b.get_output(0)
    ...
    conv2 = Conv(network, weights, cat.get_output(0), cout, 1, 1, 1, layer_name + ".cv2")
    return conv2
...
def SPPF(
    ...
    pool1 = network.add_pooling_nd(
        conv1.get_output(0), trt.PoolingType.MAX, trt.DimsHW(ksize, ksize)
    )
    ...
    pool2 = network.add_pooling_nd(
        pool1.get_output(0), trt.PoolingType.MAX, trt.DimsHW(ksize, ksize)
    )
    ...
    pool3 = network.add_pooling_nd(
        pool2.get_output(0), trt.PoolingType.MAX, trt.DimsHW(ksize, ksize)
    )
    ...
    cat = network.add_concatenation(input_tensors)
    assert cat, "Add concatenation layer failed"
    conv2 = Conv(network, weights, cat.get_output(0), c2, 1, 1, 1, layer_name + ".cv2")
    return conv2
...
def Detect(
    ...
    for i, (inp, stride) in enumerate(zip(input, s)):
        ...
        b_Conv_0 = Conv(network, weights, inp, c2, 3, 1, 1, layer_name + f".cv2.{i}.0")
        b_Conv_1 = Conv(
            network, weights, b_Conv_0.get_output(0), c2, 3, 1, 1, layer_name + f".cv2.{i}.1"
        )
        b_Conv_2 = Conv2d(
            network,
            weights,
            b_Conv_1.get_output(0),
            reg_max_x4,
            1,
            1,
            1,
            layer_name + f".cv2.{i}.2",
        )

        b_out = b_Conv_2.get_output(0)
        b_shape = network.add_constant(
            [4], np.array(b_out.shape[0:1] + (4, reg_max, -1), dtype=np.int32)
        )
        ...
        s_Conv_0 = Conv(network, weights, inp, c3, 3, 1, 1, layer_name + f".cv3.{i}.0")
        s_Conv_1 = Conv(
            network, weights, s_Conv_0.get_output(0), c3, 3, 1, 1, layer_name + f".cv3.{i}.1"
        )
        s_Conv_2 = Conv2d(
            network, weights, s_Conv_1.get_output(0), nc, 1, 1, 1, layer_name + f".cv3.{i}.2"
        )
    ...
    reg_max = (
        np.arange(0, reg_max).astype(np.float16 if fp16 else np.float32).reshape((1, 1, -1, 1))
    )
    ...
    pre_bboxes = network.add_gather(
        Matmul.get_output(0),
        network.add_constant([1], np.array([0], dtype=np.int32)).get_output(0),
        3,
    )
    ...
    Sub = network.add_elementwise(
        anchors.get_output(0), slice_x1y1.get_output(0), trt.ElementWiseOperation.SUB
    )
    assert Sub, "Add elementwise layer failed"
    Add = network.add_elementwise(
        anchors.get_output(0), slice_x2y2.get_output(0), trt.ElementWiseOperation.SUM
    )
    ...
    BBOXES = network.add_elementwise(
        Cat_bboxes_.get_output(0), strides.get_output(0), trt.ElementWiseOperation.PROD
    )
    ...
    background_class = trt.PluginField(
        "background_class", np.array(-1, np.int32), trt.PluginFieldType.INT32
    )
    box_coding = trt.PluginField("box_coding", np.array(0, np.int32), trt.PluginFieldType.INT32)
    iou_threshold = trt.PluginField(
        "iou_threshold", np.array(iou, dtype=np.float32), trt.PluginFieldType.FLOAT32
    )
    max_output_boxes = trt.PluginField(
        "max_output_boxes", np.array(topk, np.int32), trt.PluginFieldType.INT32
    )
    plugin_version = trt.PluginField("plugin_version", np.array("1"), trt.PluginFieldType.CHAR)
    score_activation = trt.PluginField(
        "score_activation", np.array(0, np.int32), trt.PluginFieldType.INT32
    )
    score_threshold = trt.PluginField(
        "score_threshold", np.array(conf, dtype=np.float32), trt.PluginFieldType.FLOAT32
    )
    ...
    batched_nms = network.add_plugin_v2(
        inputs=[BBOXES.get_output(0), SCORES.get_output(0)], plugin=batched_nms_op
    )
