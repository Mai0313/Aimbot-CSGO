...
class EngineBuilder:
    ...
    def __init__(
        self, checkpoint: Union[str, Path], device: Optional[Union[str, int, torch.device]] = None
    ) -> None:
    ...
    def __build_engine(
        ...
        self.logger.log(
            trt.Logger.WARNING,
            f"Build tensorrt engine finish.\n" f"Save in {self.weight.absolute()!s}",
        )
    ...
    def build_from_api(
        ...
        images = self.network.add_input(
            name="images", dtype=trt.float32, shape=trt.Dims4(input_shape)
        )
        assert images, "Add input failed"

        Conv_0 = Conv(self.network, state_dict, images, width_64, 3, 2, 1, "Conv.0")
        Conv_1 = Conv(self.network, state_dict, Conv_0.get_output(0), width_128, 3, 2, 1, "Conv.1")
        C2f_2 = C2f(
            self.network,
            state_dict,
            Conv_1.get_output(0),
        ...
        Conv_3 = Conv(self.network, state_dict, C2f_2.get_output(0), width_256, 3, 2, 1, "Conv.3")
        C2f_4 = C2f(
            self.network,
            state_dict,
            Conv_3.get_output(0),
        ...
        Conv_5 = Conv(self.network, state_dict, C2f_4.get_output(0), width_512, 3, 2, 1, "Conv.5")
        C2f_6 = C2f(
            self.network,
            state_dict,
            Conv_5.get_output(0),
        ...
        Conv_7 = Conv(self.network, state_dict, C2f_6.get_output(0), width_1024, 3, 2, 1, "Conv.7")
        C2f_8 = C2f(
            self.network,
            state_dict,
            Conv_7.get_output(0),
        ...
        SPPF_9 = SPPF(
            self.network, state_dict, C2f_8.get_output(0), width_1024, width_1024, 5, "SPPF.9"
        )
        ...
        Conv_16 = Conv(
            self.network, state_dict, C2f_15.get_output(0), width_256, 3, 2, 1, "Conv.16"
        )
        ...
        Conv_19 = Conv(
            self.network, state_dict, C2f_18.get_output(0), width_512, 3, 2, 1, "Conv.19"
        )
...
class TRTModule(torch.nn.Module):
    ...
    def __init_bindings(self) -> None:
        ...
        if not odynamic:
            self.output_tensor = [
                torch.empty(info.shape, dtype=info.dtype, device=self.device) for info in out_info
            ]
