...
try:
    from .utils_trt import TRTModule, blob, det_postprocess, letterbox
except ImportError as e:
...
SIDE_COLOR_MAP = {"ct": "turquoise2", "t": "yellow", "dm": "green"}


class AimAide:
    def __init__(
        self,
        screensz: tuple[int, int],
        sectionsz: int,
        grabber: str,
        infer_method: str,
        model_path: str,
        ...
        try:
            if grabber == "d3d_gpu" and not d3d_err_flag:
                print(
                    "[red]D3D GPU GRABBING IS CURRENTLY NOT WORKING AND WILL BE REWRITTEN.\nUse win32 or d3d_np!"
                )
                ...
                self._grabber = "win32"
                self.dcObj = win32ui.CreateDCFromHandle(
                ...
                self.dataBitMap.CreateCompatibleBitmap(
                    self.dcObj, self.section_size, self.section_size
                )
    ...
    def _perform_target_selection(
        self, bboxes: np.ndarray, confs: np.ndarray, labels: np.ndarray, prefer_body: bool
    ) -> tuple[bool, float, int, int, int, int, int, int, int, int]:
        target = []
        if labels.size > 1:
            ...
            if (
                self.side_target_label_map["body_ct"] or self.side_target_label_map["body_t"]
            ) in labels and (prefer_body):
                closest_body_idx = np.where(
                    (labels[closest] == self.side_target_label_map["body_ct"]).any()
                    or (labels[closest] == self.side_target_label_map["body_t"]).any()
                )[0][0]
    ...
    def _smooth_linear_aim(
        self, dx: int, dy: int, xyxywh: list, sensitivity: int, flickieness: int
    ) -> None:
        ...
        for _ in range(n_steps):
            ...
            if (x1 + w // 3 > 0) or (x2 - w // 3 < 0) or (y1 - h // 4 > 0) or (y2 + h // 4 < 0):
                pyautogui.move(
                    int(dx // smooth / MOUSE_X_MOVE_DAMPING),
                    int(dy // smooth / MOUSE_Y_MOVE_DAMPING),
                    0,
                    _pause=False,
                )
    ...
    def _benchmark(self) -> None:
        ...
        while time.perf_counter() - t1 < 5:
            ...
            print(
                f"Running in benchmark mode for 300 iterations in {(4-t):.1f} sec!",
                end="\r",
                flush=True,
            )
            time.sleep(1)
    ...
    def run(
        ...
        while not c.isSet():
            ...
            detected, conf, x1, y1, x2, y2, w, h, dx, dy = self._perform_target_selection(
                bboxes, confs, labels, prefer_body=prefer_body
            )
            ...
            if count_fps == 30:
                print(
                    f"[{self._side_color}]FPS:{(int(1/(runtime_end-runtime_start)))} | conf:{conf:.2f} dx:{abs(dx):03d} dy:{abs(dy):03d}",
                    end="\r",
                    flush=True,
                )
                count_fps = 0
            ...
            if benchmark:
                ...
                if count > 300:
                    print(
                        f"Average FPS: {np.mean(np.array(avg_fps)):.2f} (dev:{np.std(np.array(avg_fps)):.2f})                       _"
                    )
                    print(
                        "Above 60 FPS increase sensitivity if aim aiding feels unnatural (-sensitivity argument: default 1)."
                    )
                    break
