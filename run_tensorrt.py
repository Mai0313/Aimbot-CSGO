...
class AimAideTrt(AimAide):
    def __init__(
        ...
        if model_path.endswith("pt"):
            print(
                "[yellow]Specified YOLO.pt when a TensorRT engine is needed...Loading TensorRT engine!"
            )
        ...
        if not no_engine_check:
            ...
            for file in weights:
                ...
                if engine_filename not in engines:
                    ...
                    build_engine(
                        os.path.join(rel_path, pt_filename),
                        input_shape,
                        os.path.join(rel_path, pkl_filename),
                    )
                    print(f"[yellow]Removing {pkl_filename}")
                    os.remove(os.path.join(rel_path, pkl_filename))

        super().__init__(
            screensz, sectionsz, grabber, infer_method, model_path, model_config_path, side
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="AimAide with TensorRT", description="Run AimAide with TensorRT"
    )
    parser.add_argument(
        "--input_size", type=int, default=320, help="dimension of the input image for the detector"
    )
    parser.add_argument(
        "--grabber", type=str, default="win32", help="selected grabber (win32, d3d_gpu, d3d_np) "
    )
    parser.add_argument(
        "--model",
        default="models/yolov8s_csgo_mirage-320-v41-al-gen-bg.engine",
        help="selected engine (TensorRT) ",
    )
    parser.add_argument(
        "--config", type=str, default="models/config.json", help="path to model config json"
    )
    parser.add_argument(
        "--side", type=str, default="dm", help="which side your are on, ct, t or dm (deathmatch))"
    )
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="show live detector output in a new window"
    )
    parser.add_argument(
        "--view_only", action="store_true", help="run in view only mode (disarmed)"
    )
    parser.add_argument("--benchmark", action="store_true", help="launch benchmark mode")
    parser.add_argument(
        "--no_engine_check", action="store_true", help="avoid checking for missing engines"
    )
